...
#define cb_mk_vec(name, type)                                              \
	...
	extern type  *name##_push_back(name       vec[restrict static 1],  \
	                               const type val[restrict static 1]); \
	extern void   name##_clear(name vec[static 1]);                    \
	extern bool   name##_insert(name       vec[restrict static 1],     \
	                            size_t     at_index,                   \
	                            const type val[restrict static 1]);
...
#define cb_impl_vec(name, type)                                                \
	...
	size_t name##_realloc(name vec[static 1], size_t new_cap) {            \
		...
		return tmp_vec_cap;                                            \
                                                                               ...
                                                                               \
	type *name##_push_back(name       vec[restrict static 1],              \
	                       const type val[restrict static 1]) {            \
		if (vec->size >= vec->cap)                                     \
	...
	void name##_clear(name vec[static 1]) {                                \
		vec->size = 0;                                                 \
                                                                               ...
                                                                               \
	bool name##_insert(name vec[restrict static 1], size_t at_index,       \
	                   const type val[restrict static 1]) {                \
		...
		if (vec->size >= vec->cap)                                     \
			if (!name##_realloc(vec, name##_new_cap(vec->cap)))    \
				return false;                                  \
                                                                               \
		memmove(&vec->p[at_index + 1], &vec->p[at_index],              \
		        sizeof(type[size_to_move]));                           \
