...
static int kqvk_reload_vulkan(VkInstance ins, VkPhysicalDevice pdev,
                              VkDevice ldev);
#if KQ_DEBUG
static VKAPI_ATTR VkBool32 VKAPI_CALL
kq_callback_vk_debug(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
                     VkDebugUtilsMessageTypeFlagsEXT        messageType,
                     const VkDebugUtilsMessengerCallbackDataEXT *pCallbackData,
                     void                                       *pUserData);

static bool kqvk_instance_add_validation_layers(VkInstanceCreateInfo
                                                        instance_cinfo
                                                                [static 1]);
#endif /* KQ_DEBUG */
static bool kqvk_instance_add_extensions(VkInstanceCreateInfo
                                                 instance_cinfo[static 1]);
static bool kqvk_choose_pdev(kq_data kq[static 1]);
static bool kqvk_check_pdev_for_extension(VkPhysicalDevice pdev,
...
static u32  kqvk_mem_type_find(kq_data kq[static 1], u32 type_filter,
                               VkMemoryPropertyFlags props);
static void kqvk_buffer_copy(kq_data kq[static 1], VkBuffer src, VkBuffer dst,
                             VkDeviceSize size);
static bool kqvk_sync_primitives_create(kq_data kq[static 1]);
static bool kqvk_swapchain_recreate(kq_data kq[static 1]);
static bool kqvk_cmd_buf_record(kq_data kq[static 1], VkCommandBuffer cmd_buf,
                                u32 img_index);

cb_vec(vecstr, char *);

#if KQ_DEBUG
static const char *const kqvk_wanted_validation_layers[] = {
	"VK_LAYER_KHRONOS_validation"};
static const u32 kqvk_wanted_validation_layers_count =
	sizeof kqvk_wanted_validation_layers
	/ sizeof kqvk_wanted_validation_layers[0];
...
bool KQinit(kq_data kq[static 1]) {
...
#if KQ_DEBUG
	if (vkCreateDebugUtilsMessengerEXT(kq->vk_ins,
	                                   &rend_info.debug_messenger_cinfo, 0,
	                                   &kq->dbg_messenger)) {
	...
	if (vkCreateDevice(kq->vk_pdev, &rend_info.ldevice_cinfo, 0,
	                   &kq->vk_ldev)) {
...
void KQstop(kq_data kq[static 1]) {
	...
	for (size_t i = 0; i < KQ_FRAMES_IN_FLIGHT; ++i) {
		vkDestroySemaphore(kq->vk_ldev,
		                   kq->render_finished_semaphore[i], 0);
		vkDestroySemaphore(kq->vk_ldev, kq->img_available_semaphore[i],
		                   0);
...
bool KQrender(kq_data kq[static 1]) {
	...
	vkWaitForFences(kq->vk_ldev, 1, &kq->in_flight_fence[current_frame],
	                VK_TRUE, UINT64_MAX);
	...
	u32 img_index = 0U;
retry_acquire:
	switch (vkAcquireNextImageKHR(kq->vk_ldev, kq->vk_swapchain, UINT64_MAX,
	                              kq->img_available_semaphore
	                                      [current_frame],
	                              0, &img_index)) {
	...
	rend_info.submit_info.pWaitSemaphores =
		&kq->img_available_semaphore[current_frame];
	rend_info.submit_info.pSignalSemaphores =
		&kq->render_finished_semaphore[current_frame];

	if (vkQueueSubmit(kq->q_graphics, 1, &rend_info.submit_info,
	                  kq->in_flight_fence[current_frame]))
		return false;

	rend_info.present_info.pImageIndices = &img_index;
	rend_info.present_info.pWaitSemaphores =
		&kq->render_finished_semaphore[current_frame];
...
static int kqvk_reload_vulkan(VkInstance ins, VkPhysicalDevice pdev,
	...
	LOGM_DEBUG("Vulkan loaded (%s, %s, %s).",
	           ...
	           ldev ? "VkDevice" : "0");
	LOGM_INFO("Vulkan %d.%d.", GLAD_VERSION_MAJOR(vk_ver),
	          GLAD_VERSION_MINOR(vk_ver));
...
static bool kqvk_instance_add_validation_layers(VkInstanceCreateInfo
	...
	VkLayerProperties *avail_layers =
		malloc(sizeof(VkLayerProperties[avail_layers_count]));
	if (!avail_layers) {
		KQ_OOM_MSG();
		return false;
	}
	...
	for (u32 i = 0U; i < kqvk_wanted_validation_layers_count; ++i) {
		...
		for (u32 j = 0U; j < avail_layers_count; ++j) {
			if (!strcmp(avail_layers[j].layerName,
			            kqvk_wanted_validation_layers[i])) {
				found = true;
				break;
			}
		}
		...
		if (found) {
			LOGM_DEBUG("Enabling validation layer %s.",
			           kqvk_wanted_validation_layers[i]);
			if (!vecstr_push_back(kqvk_validation_layers_vec,
			                      &kqvk_wanted_validation_layers
			                              [i])) {
				KQ_OOM_MSG();
				free(avail_layers);
				vecstr_destroy(kqvk_validation_layers_vec);
			...
			LOGM_ERROR("Missing validation layer %s.",
			           kqvk_wanted_validation_layers[i]);
		}
	}
	free(avail_layers);
	...
	instance_cinfo->enabledLayerCount =
		(u32)kqvk_validation_layers_vec->size;
...
static bool kqvk_instance_add_extensions(VkInstanceCreateInfo
...
#if KQ_DEBUG
	LOGM_DEBUG("Enabling debug instance extension %s.",
	...
	instance_cinfo->enabledExtensionCount =
		(u32)kqvk_instance_exts_vec->size;
...
static bool kqvk_choose_pdev(kq_data kq[static 1]) {
	...
	for (u32 i = 0U; i < pdev_count; ++i) {
		if (kqvk_check_pdev_for_extension(
			    pdevs[i], VK_KHR_SWAPCHAIN_EXTENSION_NAME)) {
			chosen_pdev = i;
			break;
		}
	}
	...
	LOGM_INFO("\"%s\" chosen as Vulkan physical device.",
	          pdev_props.deviceName);

	vkGetPhysicalDeviceSurfaceCapabilitiesKHR(kq->vk_pdev, kq->vk_surface,
	                                          &kq->vk_surface_capabilities);

	int w, h;
	glfwGetFramebufferSize(kq->win, &w, &h);
	kqvk_ready_new_resolution(kq, w, h);

	rend_info.swapchain_cinfo.preTransform =
		kq->vk_surface_capabilities.currentTransform;
	...
	rend_info.swapchain_cinfo.minImageCount =
		kq->vk_surface_capabilities.minImageCount + KQ_FRAMES_IN_FLIGHT
		- 1;
	if (kq->vk_surface_capabilities.maxImageCount > 0
	    && rend_info.swapchain_cinfo.minImageCount
	               > kq->vk_surface_capabilities.maxImageCount)
		rend_info.swapchain_cinfo.minImageCount =
			kq->vk_surface_capabilities.maxImageCount;
...
static bool kqvk_check_pdev_for_extension(VkPhysicalDevice pdev,
	...
	VkExtensionProperties *pdev_exts =
		malloc(sizeof(VkExtensionProperties[pdev_exts_count]));
	if (!pdev_exts) {
		KQ_OOM_MSG();
		return false;
	}
	vkEnumerateDeviceExtensionProperties(pdev, 0, &pdev_exts_count,
	                                     pdev_exts);
...
static bool kqvk_set_up_pdev_queues(kq_data kq[static 1]) {
	...
	vkGetPhysicalDeviceQueueFamilyProperties(kq->vk_pdev, &q_family_count,
	                                         0);
	VkQueueFamilyProperties *q_families =
		malloc(sizeof(VkQueueFamilyProperties[q_family_count]));
	if (!q_families) {
		KQ_OOM_MSG();
		return false;
	}
	...
	for (u32 i = 0U; i < q_family_count; ++i) {
		...
		vkGetPhysicalDeviceSurfaceSupportKHR(kq->vk_pdev, i,
		                                     kq->vk_surface,
		                                     &present_supported);
	...
	if (kq->q_graphics_index == kq->q_present_index) {
		...
		rend_info.swapchain_cinfo.imageSharingMode =
			VK_SHARING_MODE_EXCLUSIVE;
	} else {
		rend_info.swapchain_cinfo.imageSharingMode =
			VK_SHARING_MODE_CONCURRENT;
		rend_info.swapchain_cinfo.queueFamilyIndexCount = 2;
		rend_info.swapchain_cinfo.pQueueFamilyIndices =
			kq->q_indices_as_array;
	}
...
static void kqvk_ready_new_resolution(kq_data kq[static 1], int w, int h) {
	...
	if (rend_info.swapchain_cinfo.imageExtent.width
	    < kq->vk_surface_capabilities.minImageExtent.width)
		rend_info.swapchain_cinfo.imageExtent.width =
			kq->vk_surface_capabilities.minImageExtent.width;
	else if (rend_info.swapchain_cinfo.imageExtent.width
	         > kq->vk_surface_capabilities.maxImageExtent.width)
		rend_info.swapchain_cinfo.imageExtent.width =
			kq->vk_surface_capabilities.maxImageExtent.width;
	if (rend_info.swapchain_cinfo.imageExtent.height
	    < kq->vk_surface_capabilities.minImageExtent.height)
		rend_info.swapchain_cinfo.imageExtent.height =
			kq->vk_surface_capabilities.minImageExtent.height;
	else if (rend_info.swapchain_cinfo.imageExtent.height
	         > kq->vk_surface_capabilities.maxImageExtent.height)
		rend_info.swapchain_cinfo.imageExtent.height =
			kq->vk_surface_capabilities.maxImageExtent.height;
	rend_info.pass_begin_info.renderArea.extent =
		rend_info.swapchain_cinfo.imageExtent;
...
static bool kqvk_swapchain_create(kq_data kq[static 1]) {
	if (vkCreateSwapchainKHR(kq->vk_ldev, &rend_info.swapchain_cinfo, 0,
	                         &kq->vk_swapchain)) {
		LOGM_FATAL("Unable to create swapchain.");
		return false;
	}

	vkGetSwapchainImagesKHR(kq->vk_ldev, kq->vk_swapchain,
	                        &kq->swapchain_img_count, 0);
	...
	vkGetSwapchainImagesKHR(kq->vk_ldev, kq->vk_swapchain,
	                        &kq->swapchain_img_count, kq->swapchain_imgs);

	kq->swapchain_img_views =
		malloc(sizeof(VkImageView[kq->swapchain_img_count]));
	...
	for (u32 i = 0U; i < kq->swapchain_img_count; ++i) {
		rend_info.swapchain_img_view_cinfo.image =
			kq->swapchain_imgs[i];
		if (vkCreateImageView(kq->vk_ldev,
		                      &rend_info.swapchain_img_view_cinfo,
		                      0,
		                      &kq->swapchain_img_views[i])) {
			LOGM_FATAL("Unable to create swapchain image view %u.",
			           i + 1);
			for (u32 j = 0U; j < i; ++j)
				vkDestroyImageView(kq->vk_ldev,
				                   kq->swapchain_img_views[j],
				                   0);
			free(kq->swapchain_img_views);
			free(kq->swapchain_imgs);
			vkDestroySwapchainKHR(kq->vk_ldev, kq->vk_swapchain, 0);
...
static bool kqvk_init_shaders(kq_data kq[static 1]) {
	size_t vert_len = 0;
	u32   *vert_buf =
		fs_file_read_all_alloc("shaders/triangle.vert.spv", &vert_len);
	if (!(vert_buf
	      && !(vert_len % 4))) { // codeSize must be a multiple of 4.
		KQ_OOM_MSG();
		return false;
	}
	...
	size_t frag_len = 0;
	u32   *frag_buf =
		fs_file_read_all_alloc("shaders/triangle.frag.spv", &frag_len);
	...
	if (vkCreateShaderModule(kq->vk_ldev,
	                         &rend_info.shader_module_vertex_cinfo, 0,
	                         &kq->vert_module)) {
	...
	if (vkCreateShaderModule(kq->vk_ldev,
	                         &rend_info.shader_module_fragment_cinfo, 0,
	                         &kq->frag_module)) {
...
static bool kqvk_render_pass_create(kq_data kq[static 1]) {
	...
	if (vkCreateRenderPass(kq->vk_ldev, &rend_info.pass_cinfo, 0,
	                       &kq->render_pass)) {
...
static bool kqvk_pipeline_create(kq_data kq[static 1]) {
	if (vkCreatePipelineLayout(kq->vk_ldev,
	                           &rend_info.pipeline_layout_cinfo, 0,
	                           &kq->pipeline_layout)) {
...
static bool kqvk_framebuffers_create(kq_data kq[static 1]) {
	...
	for (u32 i = 0U; i < kq->swapchain_img_count; ++i) {
		...
		if (vkCreateFramebuffer(kq->vk_ldev, &rend_info.fbo_cinfo, 0,
		                        &kq->fbos[i])) {
			LOGM_FATAL("Unable to create framebuffer %u.", i);
			for (u32 j = 0U; j < i; ++j)
				vkDestroyFramebuffer(kq->vk_ldev, kq->fbos[j],
				                     0);
			free(kq->fbos);
			return false;
		}
...
static bool kqvk_cmd_pool_and_buf_create(kq_data kq[static 1]) {
	if (vkCreateCommandPool(kq->vk_ldev, &rend_info.cmd_pool_cinfo, 0,
	                        &kq->cmd_pool)) {
	...
	if (vkAllocateCommandBuffers(kq->vk_ldev,
	                             &rend_info.cmd_buf_allocate_info,
	                             kq->cmd_buf)) {
...
static bool kqvk_vertex_buffer_create(kq_data kq[static 1]) {
	...
	void *mapped_buf_mem;
	vkMapMemory(kq->vk_ldev, staging_buf_mem, 0,
	            rend_info.vertex_buf_cinfo.size, 0, &mapped_buf_mem);
	memcpy(mapped_buf_mem, triangle_vertices,
	       rend_info.vertex_buf_cinfo.size);
	vkUnmapMemory(kq->vk_ldev, staging_buf_mem);

	kqvk_buffer_create(kq, rend_info.vertex_buf_cinfo.size,
	                   VK_BUFFER_USAGE_TRANSFER_DST_BIT
	                           | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
	                   VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &kq->vertex_buf,
	                   &kq->vertex_buf_mem);

	kqvk_buffer_copy(kq, staging_buf, kq->vertex_buf,
	                 rend_info.vertex_buf_cinfo.size);
...
static bool kqvk_buffer_create(kq_data               kq[restrict static 1],
	...
	VkMemoryAllocateInfo mem_ainfo =
		{.sType          = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,
	         ...
	         .memoryTypeIndex =
	                 kqvk_mem_type_find(kq, mem_reqs.memoryTypeBits,
	                                    props)};
...
static void kqvk_buffer_copy(kq_data kq[static 1], VkBuffer src, VkBuffer dst,
                             VkDeviceSize size) {
	VkCommandBufferAllocateInfo cmd_buf_ainfo =
	...
	VkCommandBufferBeginInfo cmd_buf_binfo =
...
static bool kqvk_sync_primitives_create(kq_data kq[static 1]) {
	for (size_t i = 0; i < KQ_FRAMES_IN_FLIGHT; ++i) {
		if (vkCreateFence(kq->vk_ldev, &rend_info.fence_cinfo, 0,
		                  &kq->in_flight_fence[i])) {
			for (size_t j = 0; j < i; ++j) {
				vkDestroySemaphore(kq->vk_ldev,
				                   kq->render_finished_semaphore
				                           [j],
				                   0);
				vkDestroySemaphore(kq->vk_ldev,
				                   kq->img_available_semaphore
				                           [j],
				                   0);
				vkDestroyFence(kq->vk_ldev,
				               kq->in_flight_fence[j], 0);
			}
			return false;
		}
		if (vkCreateSemaphore(kq->vk_ldev, &rend_info.semaphore_cinfo,
		                      0, &kq->img_available_semaphore[i])) {
			vkDestroyFence(kq->vk_ldev, kq->in_flight_fence[i], 0);
			for (size_t j = 0; j < i; ++j) {
				vkDestroySemaphore(kq->vk_ldev,
				                   kq->render_finished_semaphore
		...
		if (vkCreateSemaphore(kq->vk_ldev, &rend_info.semaphore_cinfo,
		                      0, &kq->render_finished_semaphore[i])) {
			vkDestroyFence(kq->vk_ldev, kq->in_flight_fence[i], 0);
			vkDestroySemaphore(kq->vk_ldev,
			                   kq->img_available_semaphore[i], 0);
			for (size_t j = 0; j < i; ++j) {
				vkDestroySemaphore(kq->vk_ldev,
				                   kq->render_finished_semaphore
				                           [j],
...
static bool kqvk_cmd_buf_record(kq_data kq[static 1], VkCommandBuffer cmd_buf,
	...
	vkCmdBeginRenderPass(cmd_buf, &rend_info.pass_begin_info,
	...
	vkCmdBindVertexBuffers(cmd_buf, 0, 1, &kq->vertex_buf,
	                       &vertex_buf_offset);
